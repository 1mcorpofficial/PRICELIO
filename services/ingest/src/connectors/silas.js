/**
 * Šilas Lithuania Connector
 * Scrapes promotional offers from silas.lt/akcijos
 */
...
const SILAS_URL = 'https://www.silas.lt/akcijos';
const STORE_CHAIN = 'Šilas';
...
function parseSilasHtml(html) {
  const $ = cheerio.load(html);
  const offers = [];

  const selectors = ['.product', '.akcija', '.offer', '.product-card', '[class*="offer-item"]'];

  for (const sel of selectors) {
    $(sel).each((_, elem) => {
      const $e = $(elem);
      const name = ($e.find('.name, .title, h3, h4').first().text() || '').trim();
      const priceRaw = $e.find('.price, .kaina').first().text();
...
async function run() {
  ...
  let html;
  try {
    const resp = await axios.get(SILAS_URL, { headers: HEADERS, timeout: 15000 });
    html = resp.data;
  } catch (err) {
    console.error('❌ Šilas fetch failed:', err.message);
    throw err;
  }

  let offers = parseSilasHtml(html);

  if (offers.length === 0) {
    console.log('⚠️  Šilas CSS parse yielded 0 offers — trying AI text extraction');
    const $ = cheerio.load(html);
  ...
  console.log(`✅ Šilas: ${offers.length} offers extracted`);
  if (offers.length === 0) return { offers_found: 0, offers_published: 0, errors: 0 };

  const result = await publishOffers(offers, STORE_CHAIN, SILAS_URL);
  console.log(`📦 Šilas published ${result.published} offers (${result.errors} errors)`);
