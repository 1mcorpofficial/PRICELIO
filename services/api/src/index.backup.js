  ...
  try {
    ...
    const result = await query(
      `WITH matched AS (
         ...
         FROM products p
         WHERE (to_tsvector('simple', p.name) @@ plainto_tsquery('simple', $1)
             OR p.name ILIKE '%' || $1 || '%')
         LIMIT 30
       ),
       daily AS (
         SELECT
           o.store_chain,
           o.fetched_at::date AS day,
           MIN(o.price_value) AS price
         FROM offers o
         JOIN matched m ON m.id = o.product_id
         WHERE o.status = 'active'
         GROUP BY o.store_chain, o.fetched_at::date
       )
       SELECT store_chain, day::text, price
       FROM daily
       ORDER BY day ASC, store_chain`,
      [q]
    );
    ...
    const series = Object.entries(chainMap).map(([chain, byDate]) => ({
      chain,
      prices: datesSet.map(d => byDate[d] ?? null),
    }));
    series.sort((a, b) => {
